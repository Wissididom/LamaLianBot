...
export default async function handleGuildMemberUpdate(
  ...
  if (oldMember.avatar != newMember.avatar) {
    ...
    embed.addFields({
      name: "Server-Avatar",
      value:
        `[vorher](<${oldMember.displayAvatarURL()}>) -> [nachher](<${newMember.displayAvatarURL()}>)`,
      inline: true,
    });
    shouldPost = true;
  }
...
function addTimeout(member: GuildMember, embed: EmbedBuilder) {
  const timeoutTimestamp = Math.floor(
    (member.communicationDisabledUntilTimestamp ?? 1000) / 1000,
  );
  return embed
...
function removeTimeout(member: GuildMember, embed: EmbedBuilder) {
  return embed
...
async function fetchRoler(member: GuildMember) {
